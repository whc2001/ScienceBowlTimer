...
A quick vibe-coded desktop application for Science Bowl competitions.

## Usage

- This application expects dual-screen for optimal usage. With two screens attached in extended mode, one will display the public facing big-screen and another will display a control panel optimized for touch-screen usage.

- The big-screen will display the current half and remaining time, as well as the current question time and countdown.
...
- Press Alt+F4 or click the close button on the bottom right of the control panel to exit the application.

- If "Time.wav", "FiveSeconds.wav" and "HalfFinished.wav" are placed in the application directory, they will be played when for corresponding events. The OS volume will be automatically set to 100% each time an audio is played. Adjust the output volume through external equipments if necessary.
